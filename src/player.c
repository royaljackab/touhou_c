...
void UpdatePlayer() {
    ...
    if (player.sprite.srcRect.y == 48) {
        if (player.sprite.srcRect.width < 0) currentAnim = REIMU_RIGHT;
        else currentAnim = REIMU_LEFT;
    }

    int targetAnim = REIMU_IDLE;
    if (move.x < 0) targetAnim = REIMU_LEFT;
    else if (move.x > 0) targetAnim = REIMU_RIGHT;
    ...
    if (currentAnim != targetAnim) {
        if (targetAnim == REIMU_IDLE) {
            ...
            if (player.sprite.srcRect.width < 0) player.sprite.srcRect.width *= -1;
        } 
